...
class FootballGame:
    ...
    @functools.partial(jax.jit, static_argnames=('self'))
    def step(self, state: State, left_player_action: Action, right_player_action: Action) -> tuple[State, jnp.number]:
        # check for goal
        goal = jnp.int8(0) # -1=went in left goal; +1=went in right goal
        ...
        # only update state if no goal
        return jax.lax.cond(goal == 0, lambda: self._update_state(state, left_player_action, right_player_action), lambda: state), goal

    @functools.partial(jax.jit, static_argnames=('self'))
    def _update_state(self, state: State, left_player_action: Action, right_player_action: Action) -> State:
    ...
    def _physics_step(self, state: State) -> State:
        ...
        def collision_iteration(carry):
            i, collider_is, cur_t, coll_t_matrix, circle_colliders = carry
            ...
            return new_i, collider_is, coll_t, coll_t_matrix, circle_colliders 

        MAX_ITERATIONS = 64

        _, _, _, _, circle_colliders = jax.lax.while_loop(
            cond_fun=lambda carry: jnp.logical_and(carry[0] < MAX_ITERATIONS, carry[2] < self.DT), 
                # when cur_t >= DT, all objects updated to DT -> done
            body_fun=collision_iteration,
            init_val=(jnp.int32(0), collider_is, coll_t, coll_t_matrix, circle_colliders)
        )
